...
* Under `InteractWeapon`, click `show example weapon` to show an example weapon for positioning and resizing your own weapon.
...
* Change the values in `RagdollHitInfoObj` and `CollisionEffect` as (shown in screenshot) on the **Parent Object**.
  (Possible options to enter here are can be found on this page: [HitInfo and CollisionEffect](../details/hitinfo-and-collisioneffect.md).)
...
(**Explanation**: To learn more about `RagdollHitInfoObj` and `CollisionEffect`, please read [HitInfo and CollisionEffect](../details/hitinfo-and-collisioneffect.md).)
...
* At last, reference these `AttachLine`s to the **InteractWeapon** component from the **Parent Object**.
![image](create-a-weapon-mod/attach-line-connect.png)

#### 10.Add MountPoint for backpack

* Create Empty object under **Parent Object**.
* Name it **MountPoint**.
* Add **AttachPoint** Component to it.
* Move it to the relative position where they should attach to backpack.
![image](create-a-weapon-mod/mount-point.png)
* Reference the **MountPoint** to the **InteractWeapon** from the **Parent Object** like before.
![image](create-a-weapon-mod/mount-point-connect.png)
